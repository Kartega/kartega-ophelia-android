package com.ahmetkilic.ophelia.ea_progress.preloaders.rectangular;
...
public class LineScalePulseOutRapid extends BasePreloader {
    ...
    public LineScalePulseOutRapid(View target, int size, int duration) {
    ...
    protected void init() {
        ...
        int mDuration = getEa_duration() == 0 ? 500 : getEa_duration();
        delays = new int[TOTAL_LINES];
        delays[0] = mDuration / 5 * 4;
        delays[1] = mDuration / 5 * 2;
        delays[2] = 0;
        delays[3] = mDuration / 5 * 2;
        delays[4] = mDuration / 5 * 4;
    ...
    private void setAnimator(final int index){
        ...
        valueAnimators[index].setDuration(getEa_duration() == 0 ? 500 : getEa_duration());
