...
public class LineScalePulseOut extends BasePreloader {
    ...
    public LineScalePulseOut(View target, int size, int duration) {
    ...
    protected void init() {
        ...
        int mDuration = getEa_duration() == 0 ? 600 : getEa_duration();
        delays = new int[TOTAL_LINES];
        delays[0] = mDuration / 3 * 4;
        delays[1] = mDuration / 3 * 2;
        delays[2] = 0;
        delays[3] = mDuration / 3 * 2;
        delays[4] = mDuration / 3 * 4;
    ...
    private void setAnimator(final int index){
        ...
        valueAnimators[index].setDuration(getEa_duration() == 0 ? 600 : getEa_duration());
