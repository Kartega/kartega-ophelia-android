...
public class BallPulseSync extends BasePreloader {
    ...
    public BallPulseSync(View target, int size, int duration) {
    ...
    protected List<ValueAnimator> setupAnimation() {
        ...
        valueAnimator1.setDuration(getEa_duration() == 0 ? SPEED : getEa_duration());
        ...
        valueAnimator2.setStartDelay(getEa_duration() == 0 ? SPEED / 3 : getEa_duration() / 3);
        valueAnimator2.setDuration(getEa_duration() == 0 ? SPEED : getEa_duration());
        ...
        valueAnimator3.setStartDelay(getEa_duration() == 0 ? SPEED / 3 * 2 : getEa_duration() / 3 * 2);
        valueAnimator3.setDuration(getEa_duration() == 0 ? SPEED : getEa_duration());
    ...
    public int getHeight() {
        ...
        switch (getSize()) {
            case CrystalPreloader.Size.VERY_SMALL:
                height = getTarget().getResources().getDimension(R.dimen.height_ball_pulse_sync_vs);
                break;
            case CrystalPreloader.Size.SMALL:
                height = getTarget().getResources().getDimension(R.dimen.height_ball_pulse_sync_s);
                break;
            case CrystalPreloader.Size.MEDIUM:
                height = getTarget().getResources().getDimension(R.dimen.height_ball_pulse_sync_m);
                break;
            case CrystalPreloader.Size.LARGE:
                height = getTarget().getResources().getDimension(R.dimen.height_ball_pulse_sync_l);
                break;
            case CrystalPreloader.Size.EXTRA_LARGE:
                height = getTarget().getResources().getDimension(R.dimen.height_ball_pulse_sync_el);
                break;
        }
