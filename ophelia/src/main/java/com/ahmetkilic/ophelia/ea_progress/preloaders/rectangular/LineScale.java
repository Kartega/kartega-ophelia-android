...
public class LineScale extends BasePreloader {
    ...
    public LineScale(View target, int size, int duration) {
    ...
    protected void init() {
        ...
        int max_delay = getEa_duration() == 0 ? 650 : getEa_duration();
        delays = new int[TOTAL_LINES];
        delays[0] = 0;
        delays[1] = max_delay / 4;
        delays[2] = max_delay / 2;
        delays[3] = max_delay / 4 * 3;
        delays[4] = max_delay;
    ...
    private void setAnimator(final int index){
        ...
        valueAnimators[index].setDuration(getEa_duration() == 0 ? 650 : getEa_duration());
